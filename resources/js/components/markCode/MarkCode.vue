...
export default {
    ...
    watch: {
        ...
        '$route.params.id'(id) {
            // Уход с карточки (назад в браузере, переход в список) — это тоже смена
            // параметра: он становится undefined, а компонент ещё жив. Без проверки
            // отсюда улетал запрос /api/mark-code/undefined и сервер отвечал 500.
            if (!id || this.$route.name !== 'mark-code') {
                return;
            }
            this.code = null;
            this.chz = null;
